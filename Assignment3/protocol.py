from __future__ import generator_stop
import Crypto.Hash.HMAC
from Crypto.Cipher import AES
import time
import random
import os

# generator and modulus chosen from rfc3526, specifically groups #14 and #15. 
# https://www.ietf.org/rfc/rfc3526.txt
generator = 2
modulus = 

# group 15 in rfc3526
generatorP = 2
modulusP = 
    
class Protocol:


    # Initializer (Called from app.py)
    # TODO: MODIFY ARGUMENTS AND LOGIC AS YOU SEEM FIT
    def __init__(self):
        self._skey = None
        self._ikey = None
        self.timeStamp = 0  # add timestamp to self object for the protocol 
        self.pNum = 0
        self.pNumP = 0
        

    # Creating the initial message of your protocol (to be send to the other party to bootstrap the protocol)
    # TODO: IMPLEMENT THE LOGIC (MODIFY THE INPUT ARGUMENTS AS YOU SEEM FIT)
    
    # Important note: so the way we implemented getProtocolInitMsg is just as a helper function.
    # Basically, since the server needs to get the timestamp from decrypting the original init msg of the client, we need to save the timestamp somewhere, 
    # then pass it to the function. In the case that the server side is using it, since we need to extract the timestamp, we just assume that the protocol will handle 
    # the act of decrypting the message received and then saving the received timestamp into the self object of the protocl which we then use to call this function.
    def GetProtocolInitiationMessage(self):
        # initializing AES-CCM mode with the shared secret inputted into the program by both users, effectively Kab. 
        key = self.sharedSecret
        cipher = AES.new(key, AES.MODE_CCM)

        # the crypto library suggests using the os random number generator and generally considers it to be a cryptographically secure random number generator: 
        # link here: https://cryptography.io/en/latest/random-numbers/
        # as for choice on the exponent bit-size, there was a section in the rfc document used above that said that the exponent should be roughly 2x the bit-strength of the group we chose (last paragraph introduction section)
        # by looking at the table, group 14 had a bit strength of 110, group 15 had 130, thus I chose 256 as the exponent bit-size for both. 
        

        if self.mode.get() == 0:    # client side init message
            a = os.urandom(256)
            aP = os.urandom(256)
            self.pNum = a
            self.pNumP = aP
            data = ("CLNT" + str(int(time.time())) + str((generator ** a) % modulus) + str((generatorP ** aP) % modulusP) + str(key)).encode('utf-8')
        if self.mode.get() == 1:    # server side init message
            b = os.urandom(256)
            bP = os.urandom(256)
            self.pNum = b
            self.pNumP = bP
            # to double clarify, somewhere in the protocol code (find the 2nd TODO in the app.py file), after decrypting the initial key establishment msg from the client, 
            # the server will have to extract the timestamp, go to self.protocol and then set its timeStamp field or whatever you want to call it (we didnt make the field yet)
            # and then just set it to the timestamp it received. Then here we can just extract it from that object and add 1 to it to create the server initiation message. 
            # which is implemented by the processReceivedProtocolMessage, aka just call that function after receiving the message. 
            data = ("SRVR" + str(self.timeStamp + 1) + str((generator ** b) % modulus) + str((generatorP ** bP) % modulusP) + str(key)).encode('utf-8')

        ciphertext, MAC_tag = cipher.encrypt_and_digest(data.encode('utf-8'))

        # realized that the mac_tag that is acquired from the encrypt_and_digest function is actually a second level mac.
        # effectively, the encrypt function (to my understanding) replicates precisely what the entire thing does, aka what AES-CCM does in this case, and then encrypt_and_digest adds a mac to that scheme
        # simply due to the fact that every single mode has an encrypt_and_digest function that can be used, 
        return "I'm Alice".encode('utf-8') + ciphertext + MAC_tag


    # Checking if a received message is part of your protocol (called from app.py)
    # TODO: IMPLMENET THE LOGIC
    def IsMessagePartOfProtocol(self, message):
        cipher = AES.new(self.sharedSecret, AES.MODE_CCM)
        return cipher.verify(message[-16:]) # rightmost 16 bytes for the MAC, this is just the default number of bytes specified for mac length when creating an AES cipher object, can be changed by setting mac_len to whichever value when calling new. 


    # Processing protocol message
    # TODO: IMPLMENET THE LOGIC (CALL SetSessionKey ONCE YOU HAVE THE KEY ESTABLISHED)
    # THROW EXCEPTION IF AUTHENTICATION FAILS
    def ProcessReceivedProtocolMessage(self, message):
        cipher = AES.new(self.sharedSecret, AES.MODE_CCM)
        ciphertext = message[9:len(message)-16] # I'm Alice represents first 9 bytes, last 16 bytes are the MAC
        plaintext = cipher.decrypt(ciphertext)
        timestamp = plaintext[4:14]             # integer time.time() timestamp is 10 bytes, SRVR is 4 bytes so we skip that part of the  msg. 
        self.timeStamp = timestamp              # set self.timeStamp to the timestamp we received so we can then calculate timeStamp + 1 in the server's response message. 



    # Setting the key for the current session
    # TODO: MODIFY AS YOU SEEM FIT
    def SetSessionKey(self, skey, ikey):
        self._skey = skey
        self._ikey = ikey
        pass


    # Encrypting messages
    # TODO: IMPLEMENT ENCRYPTION WITH THE SESSION KEY (ALSO INCLUDE ANY NECESSARY INFO IN THE ENCRYPTED MESSAGE FOR INTEGRITY PROTECTION)
    # RETURN AN ERROR MESSAGE IF INTEGRITY VERITIFCATION OR AUTHENTICATION FAILS
    def EncryptAndProtectMessage(self, plain_text):
        ctrcipher = AES.new(self._skey, AES.MODE_CTR)
        hmac = Crypto.Hash.HMAC.new(self._ikey)

        cipher_text = ctrcipher.nonce + ctrcipher.encrypt(plain_text.encode('utf-8'))

        hmac.update(cipher_text)
        cipher_text += hmac.digest()

        return cipher_text


    # Decrypting and verifying messages
    # TODO: IMPLEMENT DECRYPTION AND INTEGRITY CHECK WITH THE SESSION KEY
    # RETURN AN ERROR MESSAGE IF INTEGRITY VERITIFCATION OR AUTHENTICATION FAILS
    def DecryptAndVerifyMessage(self, cipher_text):
        ctrcipher = AES.new(self._skey, AES.MODE_CTR, nonce=cipher_text[:8])
        hmac = Crypto.Hash.HMAC.new(self._ikey)

        hmac.update(cipher_text[:-16])
        hmac.verify(cipher_text[-16:])

        plain_bytes = ctrcipher.decrypt(cipher_text[8:-16])

        return plain_bytes.decode('utf-8')

if __name__ == "__main__":

    #post key establishment test
    testProtocol = Protocol()
    testInput = "The quick brown fox jumps over the lazy dog"
    skey = Crypto.Random.get_random_bytes(16)
    ikey = Crypto.Random.get_random_bytes(16)
    testProtocol.SetSessionKey(skey, ikey)
    output = testProtocol.EncryptAndProtectMessage(testInput)
    input = testProtocol.DecryptAndVerifyMessage(output)
    assert input == testInput